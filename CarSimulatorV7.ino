...
void loop() {
  ...
  if (packetIndex > 0 && (millis() - lastRequestTime > 100) || GGfuckUpMyCode == 1) {
    ...
    //binaryString = 10111110001111111010100000010011;
    //pCharacteristic->setValue((uint8_t *)&binaryString, sizeof(binaryString));
    //pCharacteristic->notify();
    //Serial.println("Sending data via Bluetooth: " + binaryString);
    ...
    bits[3] = 0b1111111;   // 7 bits (to represent the 31st bit, the 8th bit is unused)
    ...
    bits[4] = { 0 };  // Create an array to hold the bytes (4 bytes = 32 bits)
    ...
    } else if (receivedData == "GG" || GGfuckUpMyCode == 1) {  // If 'STOP' is entered
      if (!pidExecuted) {
        isRunning = false;    // Set the flag that the system has stopped
        pidExecuted = false;  // Reset the command execution flag
